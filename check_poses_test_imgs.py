...
data_path = '/Users/daniele/Downloads/'
...
model_path_mask = os.path.join(
    data_path, 'models_' + model_type, 'obj_{:02d}.ply')
scene_info_path_mask = os.path.join(data_path, 'test_{}', '{:02d}', 'info.yml')
scene_gt_path_mask = os.path.join(data_path, 'test_{}', '{:02d}', 'gt.yml')
rgb_path_mask = os.path.join(
    data_path, 'test_{}', '{:02d}', 'rgb', '{:04d}.{}')
depth_path_mask = os.path.join(
    data_path, 'test_{}', '{:02d}', 'depth', '{:04d}.png')
...
vis_depth_path_mask = os.path.join(
    output_dir, '{:02d}_{}_{}_{:04d}_depth_diff.png')
...
for scene_id in scene_ids:
    ...
    for im_id, im_info in scene_info.items():
        if im_id % im_step != 0:
            continue
        print('scene: ' + str(scene_id) + ', device: ' +
              device + ', im_id: ' + str(im_id))
        ...
        #-----------------------------------------------------------------------
        # Load RGB image
        rgb_path = rgb_path_mask.format(
            device, scene_id, im_id, rgb_ext[device])
        ...
        vis_rgb_path = vis_rgb_path_mask.format(
        ...
        if device != 'canon':
            # Load depth image
            depth_path = depth_path_mask.format(
                device, scene_id, im_id, rgb_ext[device])
            ...
            for gt in scene_gt[im_id]:
                ...
                ren_depth_obj = renderer.render(
                    model, im_size, K, R, t, mode='depth')

                # for i in range(0, 10):
                fake = renderer.render(
                    model, im_size, K, R, t, mode='rgb')
                #fake = fake/1000.0
                cv2.imshow("depth", fake)
                cv2.waitKey(0)
                # K[0, 0] = K[0, 0]*0.8
                # K[1, 1] = K[0, 0]*0.8
                # K[0, 2] = K[0, 0]*0.8
                ...
                visible_mask = np.logical_or(
                    ren_depth == 0, ren_depth_obj < ren_depth)
