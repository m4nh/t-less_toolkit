...
class AriadneGraph(object):
    ...
    def generateBoundaryImage(self, image=None):
        if image is None:
            image = self.image
        return mark_boundaries(image, self.labels)
...
class AriadnePath(object):
    ...
    def __init__(self, ariadne=None, raw_points=np.array([])):
        ...
        if self.ariadne is None:
            if raw_points.ndim == 1:
                raw_points = raw_points.reshape((len(raw_points) / 2), 2)
    ...
    def asList(self):
        ...
        try:
            next_node = self.subgraph.successors(self.first_node)[0]
            while next_node is not None:
                path_nodes.append(next_node)
                next_node = self.subgraph.successors(next_node)[0]
        except:
            pass
    ...
    def loadPathsFromTXT(filename):
        ...
        paths = []
        for l in lines:
            path_data = np.array(map(int, l.split()))
